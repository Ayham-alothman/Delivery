...
function AddDriver() {
  ...
  async function handleSubmit(){
    try{
        const res=await Api.post(`/signup/driver`,{username:name,password:password,email:email});
        if(res.status==200){
            setName('');
            setEmail('');
            setPassword('');
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="bg-white p-6 rounded-lg shadow-md w-1/3">
        <h1 className="text-2xl font-bold mb-4 text-center">Sign up for Driver</h1>
