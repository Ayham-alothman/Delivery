...
function AddFactory() {
  ...
  async function handleSubmit(){
    try{
        const res=await Api.post(`/signup/factory`,{username:factoryName,password:password,email:email});
        if(res.status==200){
          setFactoryName('');
            setEmail('');
            setPassword('');
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="bg-white p-6 rounded-lg shadow-md w-1/3">
        <h1 className="text-2xl font-bold mb-4 text-center">Sign up for Factory</h1>
