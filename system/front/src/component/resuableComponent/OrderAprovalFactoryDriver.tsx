...
import { setaFctoryRequest,setFactoryAprofal,setSupermarkrtSendRequest } from "../../state/slices/orderSliceDriver";
import { useDispatch } from "react-redux";


function OrderAprovalFactoryDriver(prop:any){
    ...
    async function SendAprovalRequest(){
        try{
            let SenderProducts=UpdateProduct.map((e:product)=>{
                if(e.aproval==false){
                    delete e.aproval;
                    return e;
                }
            ...
            const ress=await Api.post(`/driver/aprovalfactory`,{
                idDriver:idDriver,
            ...
            if(ress.status==200){
                ...
                let {waitFactoryOrder,aprovalFactoryOrder,sendSupermarketOrder}=res.data;
    ...
    return (
        <div className="border-b-8 border-slate-800  px-2 h-auto space-y-1">
          ...
          <p className="MeduimText text-yellow-900">supermarket:{prop.factory.name}</p> 
...
export default OrderAprovalFactoryDriver;